export const urls = {
  linkedln: "https://www.linkedin.com/in/erdal-nayir-9754281b1/",
  planlamaPro: "https://github.com/ErdalNayir/planlamapro-backend",
  cycamp:
    "https://6314e747063a091c48a88f3c--unique-kataifi-950851.netlify.app/",
  cqrsApi: "https://github.com/ErdalNayir/CQRS_Pattern_with_Identity_Docker",
  webapi: "https://github.com/ErdalNayir/Asp.Net_Core_7.0_Web_Api",
  weather: "https://erdalnayirweatherapp.netlify.app/",
  note: "https://sticknoteappbyerdal.netlify.app/",
  mapDet: "https://map-detective.web.app/gamemodpage",
};

export const imgData = {
  aspNet:
    "https://upload.wikimedia.org/wikipedia/commons/thumb/7/7d/Microsoft_.NET_logo.svg/1200px-Microsoft_.NET_logo.svg.png",
  python:
    "https://upload.wikimedia.org/wikipedia/commons/thumb/c/c3/Python-logo-notext.svg/1869px-Python-logo-notext.svg.png",
  javascript:
    "https://upload.wikimedia.org/wikipedia/commons/thumb/9/99/Unofficial_JavaScript_logo_2.svg/2048px-Unofficial_JavaScript_logo_2.svg.png",
  express:
    "https://w7.pngwing.com/pngs/925/447/png-transparent-express-js-node-js-javascript-mongodb-node-js-text-trademark-logo.png",
  nodejs:
    "https://upload.wikimedia.org/wikipedia/commons/thumb/d/d9/Node.js_logo.svg/2560px-Node.js_logo.svg.png",
  flutter:
    "https://w7.pngwing.com/pngs/537/866/png-transparent-flutter-hd-logo.png",
  react:
    "https://upload.wikimedia.org/wikipedia/commons/thumb/a/a7/React-icon.svg/2300px-React-icon.svg.png",
  redux: "https://raw.githubusercontent.com/reduxjs/redux/master/logo/logo.png",
  postman: "https://cdn.worldvectorlogo.com/logos/postman.svg",
  VisualStudio:
    "https://1000logos.net/wp-content/uploads/2020/08/Visual-Studio-Logo.png",
  visualStudioCode:
    "https://logowik.com/content/uploads/images/visual-studio-code7642.jpg",
  tensorflow:
    "https://upload.wikimedia.org/wikipedia/commons/thumb/2/2d/Tensorflow_logo.svg/1915px-Tensorflow_logo.svg.png",
  selenium:
    "https://upload.wikimedia.org/wikipedia/commons/d/d5/Selenium_Logo.png",
  excel:
    "https://p.kindpng.com/picc/s/133-1335572_microsoft-excel-logo-png-transparent-png.png",
  powerpoint:
    "https://1000logos.net/wp-content/uploads/2020/08/Microsoft-PowerPoint-Logo.png",
  bash: "https://banner2.cleanpng.com/20180705/txh/kisspng-bash-shell-script-command-line-interface-z-shell-5b3df571eaf1a4.5375084915307871859623.jpg",
  linux:
    "https://logos-world.net/wp-content/uploads/2020/09/Linux-Logo-1996-present.png",
  php: "https://logowik.com/content/uploads/images/php.jpg",
  chatGpt: "https://1000logos.net/wp-content/uploads/2023/02/ChatGPT-Logo.jpg",
  html: "https://upload.wikimedia.org/wikipedia/commons/thumb/6/61/HTML5_logo_and_wordmark.svg/2048px-HTML5_logo_and_wordmark.svg.png",
  css: "https://upload.wikimedia.org/wikipedia/commons/thumb/d/d5/CSS3_logo_and_wordmark.svg/1200px-CSS3_logo_and_wordmark.svg.png",
  tailwind:
    "https://upload.wikimedia.org/wikipedia/commons/thumb/d/d5/Tailwind_CSS_Logo.svg/1024px-Tailwind_CSS_Logo.svg.png",
  boostrap:
    "https://img.freepik.com/premium-vector/bootstrap-icon-b-letter-logo_781017-7.jpg",
  excelVBA:
    "https://static-00.iconduck.com/assets.00/file-type-vba-icon-512x286-atniaiws.png",
};
